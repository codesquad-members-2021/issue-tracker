...
function IssueLine({ issue }: IssueLineProps) {
  ...
  return (
    <ListBody>
      ...
      <div>
        <IssueTitle>
          <AdjustRoundedIcon style={{ color: 'green' }} />
          &nbsp;{title}
          {labels.map((label, idx) => (
            <Lable key={'label' + idx} color={label.colorCode} desc={label.name} />
          ))}
        </IssueTitle>
        <IssueDesc>
          #{id} opened {timeChecker(time)} by {author} <MilestoneIcon />
          {milestone}
        </IssueDesc>
      </div>
    </ListBody>
  ...
  return (
    <ListBox>
      ...
      {openIssues.map((issue, idx) => (
        <IssueLine key={'issue' + idx} issue={issue} />
      ))}
    </ListBox>
  );
}
